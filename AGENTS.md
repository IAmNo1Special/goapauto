...
- `AGENTS.md` is excluded from mdformat checks because it carries YAML
  frontmatter; keep it well-formed manually.
...
- Release flow: bump version (both files above) → `chore: bump version to
  X.Y.Z` → tag `vX.Y.Z` → push `main` and the tag.
